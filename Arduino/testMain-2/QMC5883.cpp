...
void DFRobot_QMC5883::Setup(void)
{
  while (!begin())
  {
  ...
  readRaw();
  setHeadingDegrees();
  _initNorthDegree = getHeadingDegrees() - 180;
  while (_initNorthDegree < 0)
    _initNorthDegree += 360;
  if (_initNorthDegree > 360)
    _initNorthDegree -= 360;
  return;
}
...
void DFRobot_QMC5883::setHeadingDegrees(void)
{
...
float DFRobot_QMC5883::getRelativeDegrees(void)
{
  readRaw();
  setHeadingDegrees();
  float heading = v.HeadingDegress;
  heading -= _initNorthDegree;
  if (heading < 0)
    heading += 360;
  else if (heading > 360)
    heading -= 360;
